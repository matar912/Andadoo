...
// Sécurisation de l'URL de base (suppression des slashes superflus)
const rawPath = (page.props.adminPath ?? 'andadoo-portail-9f3k').replace(/^\/+|\/+$/g, '');
const adminBase = `/${rawPath}`;
...
<template>
    <AdminLayout>
        ...
        <p class="mt-1 text-sm text-paper-100/60">Vue d'ensemble de la flotte et des demandes de réservation.</p>

        <!-- Cartes statistiques -->
        <div class="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-5">
            <div class="rounded-xl2 border border-white/10 bg-forest-700 p-5">
                <p class="text-xs uppercase tracking-wide text-paper-100/50">Véhicules</p>
            ...
            <div class="rounded-xl2 border border-gold-500/30 bg-gold-500/10 p-5">
                <p class="text-xs uppercase tracking-wide text-gold-300">À valider</p>
        ...
        <div class="mt-6 flex gap-3">
            <!-- Si vous utilisez Ziggy : :href="route('admin.vehicles.create')" -->
            <Link :href="`${adminBase}/flotte`" class="btn-primary">+ Ajouter un véhicule</Link>

            <!-- Si vous utilisez Ziggy : :href="route('admin.reservations.index')" -->
        ...
        <h2 class="mt-10 font-display text-lg font-semibold text-white">Dernières demandes</h2>
            ...
            <table class="w-full min-w-[640px] text-left text-sm">
                ...
                <tbody class="divide-y divide-white/5 bg-forest-700/40">
                    <tr v-for="r in recent_reservations" :key="r.id">
                        ...
                        <td class="px-4 py-3">
                            <span class="rounded-full bg-white/10 px-2 py-1 text-xs text-paper-100/80">{{ statusLabels[r.status] ?? r.status }}</span>
                        </td>
