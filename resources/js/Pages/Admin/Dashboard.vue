...
<template>
    <AdminLayout>
        ...
        <p class="mt-1 text-sm text-sand-100/60">Vue d'ensemble de la flotte et des demandes de reservation.</p>

        <div class="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-5">
            <div class="rounded-xl2 border border-white/10 bg-night-700 p-5">
                <p class="text-xs uppercase tracking-wide text-sand-100/50">Vehicules</p>
                <p class="mt-2 font-display text-2xl font-bold text-white">{{ stats.vehicles_total }}</p>
            </div>
            <div class="rounded-xl2 border border-white/10 bg-night-700 p-5">
                <p class="text-xs uppercase tracking-wide text-sand-100/50">Disponibles</p>
                <p class="mt-2 font-display text-2xl font-bold text-white">{{ stats.vehicles_available }}</p>
            </div>
            <div class="rounded-xl2 border border-runway-500/30 bg-runway-500/10 p-5">
                <p class="text-xs uppercase tracking-wide text-runway-300">A valider</p>
                <p class="mt-2 font-display text-2xl font-bold text-runway-300">{{ stats.reservations_pending }}</p>
            </div>
            <div class="rounded-xl2 border border-white/10 bg-night-700 p-5">
                <p class="text-xs uppercase tracking-wide text-sand-100/50">En cours</p>
                <p class="mt-2 font-display text-2xl font-bold text-white">{{ stats.reservations_active }}</p>
            </div>
            <div class="rounded-xl2 border border-white/10 bg-night-700 p-5">
                <p class="text-xs uppercase tracking-wide text-sand-100/50">CA du mois</p>
        ...
        <div class="mt-6 flex gap-3">
            ...
            <Link :href="`${adminBase()}/reservations`" class="rounded-full border border-white/15 px-6 py-3 font-display text-sm font-semibold text-white hover:border-runway-400">
            ...
            <table class="w-full min-w-[640px] text-left text-sm">
                ...
                <tbody class="divide-y divide-white/5 bg-night-700/40">
                    <tr v-for="r in recent_reservations" :key="r.id">
                        ...
                        <td class="px-4 py-3">
                            <span class="rounded-full bg-white/10 px-2 py-1 text-xs text-sand-100/80">{{ statusLabels[r.status] }}</span>
                        </td>
                        <td class="px-4 py-3 text-sand-100/80">{{ r.total_price }} FCFA</td>
                    </tr>
                </tbody>
            </table>
        </div>
