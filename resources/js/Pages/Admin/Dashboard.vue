...
<template>
    <AdminLayout>
        ...
        <div class="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-6">
            ...
            <div class="rounded-xl2 border border-gold-500/30 bg-gold-500/10 p-5">
                <p class="text-xs uppercase tracking-wide text-gold-300">Paiements à vérifier</p>
                <p class="mt-2 font-display text-2xl font-bold text-gold-300">{{ stats.payments_pending }}</p>
            </div>
            <div class="rounded-xl2 border border-white/10 bg-forest-700 p-5">
        ...
        <!-- Boutons d'action rapide utilisant Ziggy -->
        <div class="mt-6 flex gap-3">
            <Link :href="route('admin.vehicles.create')" class="btn-primary">+ Ajouter un véhicule</Link>
            <Link :href="route('admin.reservations.index')" class="rounded-full border border-white/15 px-6 py-3 font-display text-sm font-semibold text-white hover:border-gold-400">
