...
// Sécurisation de l'URL de base pour éviter les erreurs de double slash (//)
...
function approve(r) {
    pending.value = r.id;
    router.patch(`${adminBase.value}/reservations/${r.id}/valider`, {}, {
        preserveScroll: true,
...
async function refuse(r) {
    const ok = await confirmDialog.value.open({
        ...
        message: `${r.client?.name ?? 'Le client'} sera informé que sa demande pour ${r.vehicle?.brand ?? ''} ${r.vehicle?.model ?? ''} n'a pas été retenue.`,
    ...
    pending.value = r.id;
    router.patch(`${adminBase.value}/reservations/${r.id}/refuser`, {}, {
        preserveScroll: true,
        onFinish: () => (pending.value = null),
    });
}
...
<template>
    <AdminLayout>
            ...
            <table class="w-full min-w-[640px] text-left text-sm">
                ...
                <tbody class="divide-y divide-white/5 bg-forest-700/40">
                    <tr v-for="r in reservations.data" :key="r.id" class="transition-colors duration-150 hover:bg-white/[0.03]">
                        <td class="px-4 py-3 text-white">{{ r.client?.name ?? 'Client inconnu' }}</td>
                        <td class="px-4 py-3 text-paper-100/80">{{ r.vehicle?.brand }} {{ r.vehicle?.model }}</td>
                        <td class="px-4 py-3 text-paper-100/60">
                            {{ r.start_at ? r.start_at.slice(0, 10) : '—' }} → {{ r.end_at ? r.end_at.slice(0, 10) : '—' }}
                        </td>
                        <td class="px-4 py-3">
                            <span class="rounded-full px-2 py-1 text-xs" :class="statusStyle[r.status] ?? 'bg-white/10 text-white'">
                        ...
                        <td class="px-4 py-3 text-right">
                            <template v-if="r.status === 'en_attente'">
                                ...
                                <template v-else>
                                    <button @click="approve(r)" class="mr-3 font-display text-emerald-400 transition-colors hover:text-emerald-300">
                                        Valider
                                    </button>
                                    <button @click="refuse(r)" class="font-display text-gold-400 transition-colors hover:text-gold-300">
                                        Refuser
                                    </button>
                                </template>
                            </template>
        ...
        <div v-else class="mt-16 flex flex-col items-center rounded-xl2 border border-dashed border-white/10 py-16 text-center">
            ...
            <p class="mt-1 max-w-sm text-sm text-paper-100/50">
                Les demandes des clients apparaîtront ici dès qu'ils réserveront un véhicule disponible.
            </p>
        </div>
    </AdminLayout>
</template>
