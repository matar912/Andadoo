...
<template>
    ...
    >
        <template #aside>
            <ul class="space-y-3">
                <li v-for="f in features" :key="f" class="flex items-start gap-2 text-sm text-sand-100/80">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="mt-0.5 h-4 w-4 shrink-0 text-runway-400">
        ...
        <h2 class="mt-2 font-display text-2xl font-bold text-night-500">Se connecter</h2>
        <p class="mt-1 text-sm text-night-500/60">Accédez à votre espace pour réserver.</p>

        <div v-if="status" class="mt-4 rounded-lg bg-runway-500/10 px-4 py-2 text-sm text-runway-700">
            {{ status }}
        </div>
        ...
        <form @submit.prevent="submit" class="mt-7 space-y-5">
            ...
            <div class="flex items-center justify-between text-sm">
                <label class="flex items-center gap-2 text-night-500">
                    <input v-model="form.remember" type="checkbox" class="rounded border-night-500/30 text-runway-500 focus:ring-runway-500" />
                    Rester connecté
                </label>
                <Link href="/forgot-password" class="font-display font-medium text-night-500 hover:text-runway-600">Mot de passe oublié ?</Link>
            </div>
        ...
        <p class="mt-8 text-center text-sm text-night-500/70">
            Pas encore de compte ?
            <Link href="/register" class="font-display font-semibold text-runway-600 hover:text-runway-700">Créer un compte</Link>
        </p>
    </AuthSplitLayout>
</template>
