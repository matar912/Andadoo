...
<template>
    <AppLayout>
        <section class="mx-auto max-w-6xl px-6 py-14">
            <h1 class="font-display text-3xl font-bold text-night-500">La flotte GO'CAR</h1>
            <p class="mt-2 text-night-500/70">Tous les véhicules appartiennent à GO'CAR et sont entretenus par nos équipes.</p>
            ...
            <div class="mt-6 flex flex-wrap gap-2">
                <button
                    ...
                    :class="filters.category === c ? 'border-runway-500 bg-runway-500 text-white' : 'border-night-500/15 text-night-500 hover:border-runway-400'"
                >
                    {{ c }}
                </button>
            </div>
            ...
            <div v-if="loading" class="mt-10 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
                <div v-for="n in 6" :key="n" class="card overflow-hidden">
                    <div class="h-40 animate-pulse bg-night-500/10" />
                    <div class="space-y-3 p-5">
                        <div class="h-4 w-2/3 animate-pulse rounded bg-night-500/10" />
                        <div class="h-3 w-1/2 animate-pulse rounded bg-night-500/10" />
                        <div class="h-8 w-full animate-pulse rounded bg-night-500/10" />
                    </div>
                </div>
            </div>
            ...
            <div v-else-if="vehicles.data.length" class="mt-10 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
                <div v-for="vehicle in vehicles.data" :key="vehicle.id" class="card overflow-hidden transition hover:shadow-md">
                    <Link :href="`/vehicules/${vehicle.id}`" class="flex h-40 items-center justify-center bg-sand-100">
                        ...
                        <span v-else class="font-display text-night-300">{{ vehicle.brand }} {{ vehicle.model }}</span>
                    </Link>
                    <div class="p-5">
                        <p class="font-display text-lg font-semibold text-night-500">{{ vehicle.brand }} {{ vehicle.model }}</p>
                        <p class="mt-1 text-sm capitalize text-night-500/60">{{ vehicle.category }} &middot; {{ vehicle.seats }} places &middot; {{ vehicle.transmission }}</p>
                        <div class="mt-4 flex items-center justify-between">
                            <span class="font-display text-xl font-bold text-runway-600">{{ vehicle.daily_price }} FCFA<span class="text-sm font-normal text-night-500/50">/jour</span></span>
            ...
            <div v-else class="mt-16 flex flex-col items-center rounded-xl2 border border-dashed border-night-500/15 py-16 text-center">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.2" class="h-12 w-12 text-night-500/25">
                ...
                <p class="mt-4 font-display font-semibold text-night-500">Aucun véhicule pour ce filtre</p>
                <p class="mt-1 max-w-sm text-sm text-night-500/60">
